...
?>
<div x-data="{ open: true, selectedRoom: @entangle('selectedRoom'), showModal: false }">
    <!-- Chatbox Section -->
    <div :class="{'-translate-y-0': open, 'translate-y-full': !open}" class="fixed transition-all duration-300 transform bottom-12 right-12 h-80 w-80">
        <div class="mb-2">
            <button @click="open = !open"
                ...
                Mesajlar
                <svg x-show="!open" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 24 24" stroke="currentColor">
        ...
        <div class="w-full h-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded overflow-auto flex flex-col px-2 py-4">
            ...
            <div class="mb-2">
                ...
                <select id="roomSelect" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm" wire:model="selectedRoom">
    ...
    <div x-show="showModal" @click.away="showModal = false" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50" x-transition>
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg relative">
            <h2 class="text-lg font-semibold mb-4 text-white">Yeni Sohbet Odası Ekle</h2>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Dinleyiciyi başlatan fonksiyon
            ...
            window.currentEchoChannel = window.Echo.private(`room.${roomId}`)
                .listen('MessageSent', (e) => {
                    ...
                    if (e.room_id === parseInt(roomId)) {
                        // Burada yeni mesajı UI'a ekleyebilirsiniz
        ...
        // Oda değiştiğinde meta etiketi ve dinleyiciyi güncelleyen fonksiyon
        ...
        // Oda seçimi değiştiğinde çalışacak olan event listener
        ...
        // İlk yüklemede varsayılan oda dinleyicisini başlat
